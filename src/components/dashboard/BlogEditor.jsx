...
// import { getBlogById, updateBlog, regenerateTitle, rephraseBlog } from "../../services/blog"

// // ── Shared renderer (same as generator) ─────────────────────────────────────
...
//   const [current, setCurrent]     = useState({ title: "", content: "", prompt: "", tone: "", length_preference: "", keywords: "" })
...
//   // ── Regenerate Title — auto-saves to DB ───────────────────────────────────
//   const handleRegenTitle = async () => {
//     if (!blog?.id) return
//     try {
//       setRegenTitleLoading(true)
//       // Parse keywords from stored comma string
//       const kws = current.keywords
//         ? current.keywords.split(",").map(k => k.trim()).filter(Boolean)
//         : undefined

//       const newTitle = await regenerateTitle({
//         prompt:          current.prompt || current.title,
//         article_content: current.content,
//         keywords:        kws,
//       })

//       setCurrent(prev => ({ ...prev, title: newTitle }))

//       // ✅ Auto-patch DB immediately
//       await updateBlog(blog.id, { title: newTitle })
//       showMsg("✓ Title regenerated & saved")
//     } catch (err) { console.error(err); showMsg("✗ Title regen failed") }
//     finally { setRegenTitleLoading(false) }
//   }

//   // ── Rephrase / Rearrange / Regenerate — auto-saves to DB ──────────────────
...
//               statusMsg.startsWith("✓") ? "bg-green-50 text-green-600"
...
//           {current.prompt && (
//             <p className="text-xs text-gray-400 italic mb-2">Topic: {current.prompt}</p>
//           )}
