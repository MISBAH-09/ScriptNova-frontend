...
export default function Dashboard() {
  ...
  const renderPage = () => {
    switch (page) {
      case "manage":
        return <BlogManager posts={posts} setPage={setPage} setCurrentEdit={setCurrentEdit} />;

      case "editor":
        return <BlogEditor blog={currentEdit} setPosts={setPosts} setPage={setPage} />;

      case "humanize":
        return <Humanize setPage={setPage} setCurrentEdit={setCurrentEdit} />;
    ...
    <div className="flex h-screen bg-gray-50 text-slate-900 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 bg-slate-200">
          {/* Subscription status banner — shows plan, usage, and upgrade buttons */}
          <SubscriptionBanner />
          {renderPage()}
        </main>
      </div>
